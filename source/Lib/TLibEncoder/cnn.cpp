...
void CNN::WrapInputLayer(std::vector<cv::Mat>* input_channels,int num_blob ){
   Blob<float>* input_layer = net_->input_blobs()[num_blob];
...
float* CNN::ProcessImg2Blobs(const cv::Mat& img , const cv::Mat& images,bool uv , int num_poc){
  Blob<float>* input_blob1 = net_->input_blobs()[0];
  Blob<float>* input_blob2 = net_->input_blobs()[1];
  input_blob1->Reshape(1,num_channels_,input_geometry_.height,input_geometry_.width);
  input_blob2->Reshape(1,input_blob2->channels(),input_blob2->height(),input_blob2->width());
  net_->Reshape();
  
  std::vector<cv::Mat> input_channels;
  std::vector<cv::Mat> input_channels2;
  WrapInputLayer(&input_channels,0);
  WrapInputLayer(&input_channels2,1);
  ...
  if ( input_blob2->channels() == 5 )
    images.convertTo(img_input2, CV_32FC(5));
  else if( input_blob2->channels() == 10 )
    images.convertTo(img_input2, CV_32FC(10));

  cv::split(img_input2,input_channels2);
